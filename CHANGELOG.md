...
## [2.9.0](https://github.com/google/adk-python/compare/v2.8.0...v2.9.0) (2026-09-09)


### Highlights

This release enhances agent reliability and interactivity by introducing automatic model failovers, declarative YAML graph workflows, real-time voice integration via LiveKit, and support for MCP SDK 2.x.

* **Models**: Automatically fall back to alternative models when a primary model fails or experiences rate limits. ([c300b8d](https://github.com/google/adk-python/commit/c300b8ded62b8bc4168cdede4c85bec66d1aa8ee))
* **Integrations**: Build high-performance voice and telephony applications using the new LiveKit runner. ([f277d91](https://github.com/google/adk-python/commit/f277d9117781fb4e2d8cd2608a1d484c29884e60))
* **Workflows**: Simplify complex agent flow development by loading structured graph workflows from YAML configuration files. ([2a9461b](https://github.com/google/adk-python/commit/2a9461b1da1c3f7bf732cb37ac43a865f907c6a4))
* **MCP**: Leverage Model Context Protocol (MCP) SDK 2.x features alongside existing 1.x installations. ([856acf2](https://github.com/google/adk-python/commit/856acf21e3155d2145f9af0709434dd46843539c))

#### Breaking changes

* **Nested Workflows**: Adjust your configurations if your workflows rely on immediate propagation of failures from internal nodes without retry.
* **GCS Tool**: Configure a dedicated root directory if your GCS tools must read or write files outside of the newly confined local path boundaries.
* **Workflow Resumption**: Expect previously failed nodes to rerun on workflow resumption instead of skipping or replaying as complete.
* **Session Management**: Update error-handling logic to gracefully catch the newly raised `SessionNotFoundError` when attempting to write to missing sessions.
* **MCP SDK 2.x**: Relocate server-sent custom vendor extension fields under `_meta` to prevent validation discards under MCP SDK 2.x.
